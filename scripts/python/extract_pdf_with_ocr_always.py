...
Always converts pages to images first, then uses Tesseract OCR.
...
def extract_pdf_with_ocr(pdf_path, output_json_path, zoom=2.0, max_workers=4, max_pages=None):
    """Extract all pages using OCR with parallel processing."""
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python extract_pdf_with_ocr_always.py <input.pdf> <output.json> [zoom] [workers] [max_pages]", file=sys.stderr)
        sys.exit(1)
